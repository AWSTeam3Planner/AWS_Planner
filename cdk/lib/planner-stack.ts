...
export class PlannerStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: PlannerStackProps) {
    ...
    const lambdaStack = new PlannerLambdaStack(this, `${SYSTEM_NAME}-lambdaStack`,props);
    props.lambdaStack = lambdaStack;

    const dynamodbStack = new PlannerDynamoDBStack(this, `${SYSTEM_NAME}-dynamodbStack`,props);
    props.dynamoStack = dynamodbStack;

    const APIgatewayStack = new PlannerAPIgatewayStack(this,`${SYSTEM_NAME}-APIgatewayStack`,props);
    props.gatewayStack = APIgatewayStack;

    const cognitoStack = new PlannerCognitoStack(this,`${SYSTEM_NAME}-cognitoStack`,props);
